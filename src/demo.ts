...
const host: Host = { id:'host.tend.notes',user:{id:'demo-user',name:'You',role:'user'},onUnmount(){},documents:{version:1,
  ...
  attachments:{async upload(_id,file){const bytes=await file.arrayBuffer();const hash=[...new Uint8Array(await crypto.subtle.digest('SHA-256',bytes))].map(v=>v.toString(16).padStart(2,'0')).join('');const ext=file.type==='application/pdf'?'pdf':file.type.includes('png')?'png':file.type.includes('jpeg')?'jpg':file.type.includes('wav')?'wav':'webm';const path='attachments/'+hash+'.'+ext;attachments.set(path,file);return{path,type:file.type};},async read(_id,path){const blob=attachments.get(path);if(!blob)throw new Error('Sample attachment unavailable after reload');return blob;}},
  ...
  async create(input){state.todayCreateCalls++;const all=get();const exists=all.find(d=>d.libraryId===input.libraryId&&d.name===input.name);if(exists){if(exists.content===input.content)return exists;throw Object.assign(new Error('A note with this name already exists.'),{status:409});}if(input.name.startsWith('Daily ')&&(state.todayCreateConflict||state.todayCreateLostResponse)){const lost=state.todayCreateLostResponse;state.todayCreateConflict=false;state.todayCreateLostResponse=false;const content='# Winning daily note\n\nA concurrent writer kept this text.';const winner={...input,id:crypto.randomUUID(),content,revision:await revision(content),size:content.length,modifiedAt:Math.floor(Date.now()/1000)};put([...all,winner]);if(lost)throw Object.assign(new Error('Demo connection interrupted'),{status:503});throw Object.assign(new Error('A note with this name already exists.'),{status:409});}const d={...input,id:crypto.randomUUID(),revision:await revision(input.content),size:input.content.length,modifiedAt:Math.floor(Date.now()/1000)};put([...all,d]);return d;},
