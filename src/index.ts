...
export function activate(host: Host) {
  return { mount(container: HTMLElement) {
    ...
    const help=()=>instance.showHelp();
    container.addEventListener('tend-notes:help',help);
    container.dataset.notesHelpAction='1';
    ...
    return { async unmount() { observer.disconnect(); container.removeEventListener('tend-notes:help',help); delete container.dataset.notesHelpAction; container.removeEventListener('tend-notes:new-note',newNote); delete container.dataset.notesHeaderAction; await instance.flush?.(); await unmount(instance); } };
  } };
}
