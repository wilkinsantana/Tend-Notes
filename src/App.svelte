<script lang="ts">
  ...
  async function jumpToOutline(heading: OutlineHeading) {
    ...
    if (!target) return;
    target.focus();
    target.setSelectionRange(heading.offset, heading.offset);
    ...
    Object.assign(mirror.style, { position: 'fixed', visibility: 'hidden', pointerEvents: 'none', boxSizing: 'border-box', width: `${target.clientWidth}px`, whiteSpace: 'pre-wrap', border: '0' });
    mirror.setAttribute('aria-hidden', 'true');
    mirror.append(document.createTextNode(target.value.slice(0, heading.offset)));
    const marker = document.createElement('span');
    marker.textContent = target.value.slice(heading.offset) || '\u200b';
