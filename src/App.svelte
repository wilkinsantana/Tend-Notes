...
{#snippet tool20()}<button class="icon" title="PDF · upload attachment" aria-label="Attach PDF" onclick={() => openMedia('document')}><FileText size={17}/></button>{/snippet}
        {#if mode !== 'preview'}<div class="formatting"><ResponsiveToolbar groups={[
...
.speech-actions{display:flex;align-items:center;flex-shrink:0;gap:2px;padding:3px;border:1px solid var(--line);border-radius:9px;background:color-mix(in srgb,var(--wash) 45%,transparent)}.speech-actions:empty{display:none}@container(max-width:680px){.speech-actions .icon{width:44px;height:44px}}
</style>
