...
export function renderDocument(content: string): {html: string; media: Media[]} {
  ...
  const html = DOMPurify.sanitize(parser.parse(content) as string, {
    ALLOWED_TAGS:['p','br','hr','h1','h2','h3','h4','h5','h6','strong','em','del','blockquote','ul','ol','li','pre','code','a','table','thead','tbody','tr','th','td','input','button'],
    ALLOWED_ATTR:['href','title','type','checked','disabled','start','align','rel','target','data-notes-media'],
    ADD_URI_SAFE_ATTR:['data-notes-media','type'],
