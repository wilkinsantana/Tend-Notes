<script lang="ts">
  ...
  let {phase, scope, progress, error, onpause, onresume, onstop}: {
...
<div class="read-controls" role={error ? 'alert' : 'status'}>
  ...
  {:else}<span class="scope">{scope}</span><span>{phase === 'starting' ? 'Preparing speech…' : phase === 'paused' ? 'Paused' : 'Reading aloud'}{progress.total ? ` · ${percent}%` : ''}</span>{/if}
...
<style>
  .read-controls{min-height:42px;display:flex;align-items:center;gap:10px;padding:6px 18px;border-top:1px solid var(--line);background:color-mix(in srgb,var(--accent) 7%,var(--paper));color:var(--soft);font-size:10px}.scope{color:var(--ink);font-weight:600}.read-controls>div{display:flex;align-items:center;gap:5px;margin-left:auto}.read-controls button{display:grid;place-items:center;min-width:32px;height:30px;border:1px solid var(--line);border-radius:6px;background:var(--paper);color:var(--ink);font-size:10px}:global(.spin){animation:spin 1s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}@media(prefers-reduced-motion:reduce){:global(.spin){animation:none}}
