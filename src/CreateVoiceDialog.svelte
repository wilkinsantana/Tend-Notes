<script lang="ts">
  ...
  let label = $state(''), consent = $state(false), sampleName = $state(''), sampleSeconds = $state<number|null>(null);
  let sampleInput: HTMLInputElement | undefined;
  const sampleScript = 'Today I am taking a little time to collect my thoughts. Some ideas are simple, and others surprise me. What will I discover next? Let us find out together.';
  ...
  function wavDuration(wav:ArrayBuffer){
    if(wav.byteLength<44)return null;const view=new DataView(wav);
    const ascii=(at:number,length:number)=>String.fromCharCode(...new Uint8Array(wav,at,length));
    if(ascii(0,4)!=='RIFF'||ascii(8,4)!=='WAVE')return null;
    let at=12,byteRate=0,dataBytes=0,pcm=false;
    while(at+8<=wav.byteLength){const id=ascii(at,4),length=view.getUint32(at+4,true),start=at+8;if(start+length>wav.byteLength)return null;if(id==='fmt '&&length>=16){pcm=view.getUint16(start,true)===1;byteRate=view.getUint32(start+8,true);}else if(id==='data'){dataBytes=length;}at=start+length+(length%2);}
    const seconds=dataBytes/byteRate;return pcm&&byteRate>0&&dataBytes>0&&Number.isFinite(seconds)?seconds:null;
  }
  ...
  function clearSample(){
    ...
    if(sampleWav) new Uint8Array(sampleWav).fill(0); sampleWav=null; sampleName=''; sampleSeconds=null; consent=false;
  }
  function keepSample(wav: ArrayBuffer, name: string){
    const seconds=wavDuration(wav);
    if(seconds===null){new Uint8Array(wav).fill(0);error='Choose an uncompressed PCM WAV file.';return;}
    if(seconds<.5||seconds>30){new Uint8Array(wav).fill(0);error='Choose a sample between half a second and 30 seconds.';return;}
    clearSample(); sampleWav=wav; memoryAudioBlob=new Blob([wav],{type:'audio/wav'}); sampleUrl=URL.createObjectURL(memoryAudioBlob); sampleName=name; sampleSeconds=seconds; success=''; error='';
...
<div class="voice-layer" role="presentation"><div class="voice-dialog" role="dialog" aria-modal="true" aria-label="Create my voice" tabindex="-1" use:focusDialog>
  ...
  {:else}
    {#if setup?.model !== 'ready'}<section><h3>Prepare voice creation</h3><p>A one-time download of about {Math.ceil((setup?.modelBytes.total ?? 0)/1024/1024)} MB is needed before this device can create a personal reading voice. It starts only when you choose.</p>{#if phase==='preparing'}<div class="progress-copy"><span>{progress?.phase ?? 'Preparing'}…</span><span>{progress?.totalBytes?`${percent}%`:''}</span></div><progress max={progress?.totalBytes||1} value={progress?.completedBytes||0}></progress><button onclick={cancelOperation}>Cancel</button>{:else}<button class="download-action" onclick={()=>void prepare()} disabled={busy}><Download size={15}/> Prepare voice creation</button>{/if}</section>
    {:else}<section><h3>Make it sound like you</h3><div class="language-row"><label for="voice-sample-language">Sample language<select id="voice-sample-language" disabled><option>English</option></select></label><p>English is currently supported for downloaded personal voices.</p></div><div class="reading-script"><strong>Read this in your natural voice</strong><blockquote>{sampleScript}</blockquote><p>Find a quiet place, keep the microphone a comfortable distance away, and speak at your usual pace. Avoid music and other voices.</p></div><p>Speak naturally for 5–15 seconds. Record up to 30 seconds, or upload a PCM WAV file up to 10 MB.</p><div class="sample-actions"><button onclick={()=>void startRecording()} disabled={busy}><Mic size={15}/> Record sample</button><button onclick={()=>sampleInput?.click()} disabled={busy}><Upload size={15}/> Upload WAV</button><input bind:this={sampleInput} id="personal-voice-wav" class="sr-only" type="file" accept=".wav,audio/wav,audio/x-wav" onchange={uploaded} disabled={busy}/></div>
      ...
      {#if sampleWav}<div class="sample-ready"><Check size={16}/><span><strong>Sample ready</strong><small>{sampleName}{sampleSeconds!==null?` · ${sampleSeconds.toFixed(1)} seconds`:''}</small></span><button onclick={()=>void previewSample()} disabled={busy}><Play size={14}/> Preview sample</button><button onclick={clearSample} disabled={busy}>Replace</button></div>
...
<style>
  .language-row{display:flex;align-items:center;gap:16px;margin-top:12px}.language-row label{flex:0 0 140px;margin:0;font-size:12px}.language-row select{display:block;box-sizing:border-box;width:100%;min-height:44px;margin-top:6px;padding:8px 12px;border:1px solid var(--line);border-radius:8px;background:var(--paper);color:var(--ink);font:inherit;opacity:1}.reading-script{margin-top:16px;padding:14px;border:1px solid var(--line);border-radius:10px;background:var(--paper);font-size:13px}.reading-script strong{font-weight:600}.reading-script blockquote{margin:8px 0;line-height:1.65;color:var(--ink)}.voice-dialog button:focus-visible{outline:2px solid var(--accent);outline-offset:3px}
  .voice-layer{position:absolute;inset:0;z-index:3;padding:12px;display:grid;place-items:center;background:color-mix(in srgb,var(--paper) 68%,transparent);backdrop-filter:blur(3px)}.voice-dialog{width:min(560px,100%);max-height:100%;overflow:auto;padding:20px;background:var(--paper);color:var(--ink);border:1px solid var(--line);border-radius:14px;box-shadow:0 20px 70px #0004}header{display:flex;align-items:flex-start;gap:12px}header>div{flex:1}h2,h3,p{margin:0}h2{font-size:19px}h3{font-size:14px}header p,section p{margin-top:5px;color:var(--soft);font-size:12px;line-height:1.6}.icon-close{width:44px;height:44px;border:0;background:none;color:var(--ink)}section{margin-top:16px;padding:15px;border:1px solid var(--line);border-radius:10px;background:var(--wash)}button{box-sizing:border-box;font-family:inherit;display:inline-flex;align-items:center;justify-content:center;gap:6px;height:44px;min-height:44px;margin:0;padding:8px 11px;border:1px solid var(--line);border-radius:7px;background:var(--paper);color:var(--ink);font-size:13px;line-height:1.25;cursor:pointer}.primary{margin-top:12px;border-color:transparent;background:var(--accent);color:var(--accent-ink)}.download-action{margin-top:12px;border-color:color-mix(in srgb,var(--warning,#d7ac64) 72%,var(--ink));background:var(--warning,#d7ac64);color:var(--color-warning-content,#211704)}button:disabled,.disabled{opacity:.45;cursor:not-allowed}.sample-actions{display:flex;align-items:center;flex-wrap:wrap;gap:7px;margin-top:12px}.sample-ready,.recording,.working,.loading{display:flex;flex-wrap:wrap;align-items:center;gap:8px;margin-top:12px;font-size:11px}.sample-ready>span,.saved>div{display:grid;flex:1;min-width:0}.sample-ready small,.saved small{color:var(--soft);font-size:10px;overflow:hidden;text-overflow:ellipsis}.record-dot{width:8px;height:8px;border-radius:50%;background:var(--danger)}label{display:block;margin-top:12px;font-size:11px}input[type="text"],input:not([type]){width:100%}#personal-voice-label,#confirm-personal-voice{box-sizing:border-box;width:100%;margin-top:5px;padding:10px;border:1px solid var(--line);border-radius:7px;background:var(--paper);color:var(--ink)}.consent{display:flex;align-items:flex-start;gap:7px;line-height:1.4}.consent input{margin-top:1px;accent-color:var(--accent)}.saved-list{display:grid;gap:7px;margin-top:11px}.saved{display:flex;align-items:center;gap:7px;padding:8px;border-radius:8px;background:var(--paper)}.remove,.error{color:var(--danger)}.confirm>div{display:flex;gap:7px;margin-top:10px}.success{margin-top:13px;color:var(--accent);font-size:11px}.error{margin-top:13px;font-size:11px;line-height:1.45}.progress-copy{display:flex;justify-content:space-between;margin-top:12px;color:var(--soft);font-size:10px}progress{display:block;width:100%;height:8px;margin:7px 0 11px;accent-color:var(--accent)}.stop-preview{margin-top:12px}.sr-only{position:absolute!important;width:1px!important;height:1px!important;padding:0!important;margin:-1px!important;overflow:hidden!important;clip:rect(0,0,0,0)!important;white-space:nowrap!important;border:0!important}:global(.spin){animation:spin 1s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}@media(prefers-reduced-motion:reduce){:global(.spin){animation:none}}@container(max-width:520px){.language-row{align-items:flex-start;flex-direction:column;gap:6px}.language-row label{flex:auto;width:100%}.language-row select{font-size:16px}.sample-ready>span{flex-basis:calc(100% - 30px)}.sample-ready button{flex:1}.reading-script{padding:12px}.voice-layer{padding:6px}.voice-dialog{padding:16px}.sample-actions>*{flex:1}.saved{align-items:stretch;flex-wrap:wrap}.saved>div{flex-basis:100%}.saved button{flex:1}#personal-voice-label,#confirm-personal-voice{font-size:16px}}
</style>
