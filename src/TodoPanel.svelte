...
<section class="todo-panel" aria-labelledby="todo-title">
  ...
  <div class="tools">
    <label class="search">
      ...
      <input bind:value={query} type="search" placeholder="Search tasks" disabled={busy} />
    </label>
    <div class="status-filter" aria-label="Task status">
      <button type="button" class:active={status === 'open'} aria-pressed={status === 'open'} onclick={() => status = 'open'} disabled={busy}>Open</button>
      <button type="button" class:active={status === 'completed'} aria-pressed={status === 'completed'} onclick={() => status = 'completed'} disabled={busy}>Completed</button>
      <button type="button" class:active={status === 'all'} aria-pressed={status === 'all'} onclick={() => status = 'all'} disabled={busy}>All</button>
    </div>
  </div>
  ...
  <div class="task-list" aria-live="polite" aria-busy={loading || busy}>
              ...
              <ul>
                {#each note.rows as row (row.key)}
                  <li class:completed={row.checked}>
                    ...
                    <button class="open-note" type="button" onclick={() => onopen(row.key)} disabled={busy || loading} aria-label={`Open ${row.noteName}, task on line ${row.line}`}>Open note</button>
                  </li>
                {/each}
              </ul>
            </div>
    ...
    {:else if allDone && !hasActiveFilter}
      <div class="empty"><CheckCircle2 size={28} aria-hidden="true" /><h2>Everything is complete</h2><p>Completed tasks remain here when you want to look back.</p><button type="button" onclick={() => status = 'completed'}>Show completed</button></div>
