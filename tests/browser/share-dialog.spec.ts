...
test('an owner saves before creating a forwardable link, then can open and recover the shared session', async ({page}) => {
  ...
  await page.evaluate(async () => {
    ...
    const fixture={saved:0,created:null as any,stopped:0};
    const sharing={version:1 as const,
      ...
      async recovery(){return {savedContent:'Saved copy',currentContent:'Current copy',candidateContent:'Candidate copy',status:'source_changed'};},
    };
  ...
  await expect(page.getByRole('button',{name:'Download current copy',exact:true})).toBeVisible();
